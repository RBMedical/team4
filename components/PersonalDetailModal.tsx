...
export function PersonalDetailModal({ open, initialHn = "", onClose }: Props) {
  ...
  // โหลด test names เมื่อ modal เปิด
  React.useEffect(() => {
    if (open) loadTestNames();
    else { setShowAdd(false); setExtraItems([]); setExtraFilter([]); setSaveMsg(""); }
  }, [open]);
  ...
  function addExtraRow() {
    ...
    setExtraFilter(prev => ["", ...prev]);
  }

  function updateExtraItem(idx: number, val: string) {
    setExtraItems(prev => prev.map((v, i) => i === idx ? val : v));
    setExtraFilter(prev => prev.map((v, i) => i === idx ? val : v));
  }

  function removeExtraItem(idx: number) {
    setExtraItems(prev => prev.filter((_, i) => i !== idx));
    setExtraFilter(prev => prev.filter((_, i) => i !== idx));
  }

  async function saveExtra() {
    const items = extraItems.filter(v => v.trim());
    console.log("[saveExtra] data:", data?.HN, data?.["ชื่อ นามสกุล"], "items:", items);
    if (!items.length || !data) {
      console.log("[saveExtra] aborted — items empty or no data");
      return;
    }
    ...
    try {
      ...
      console.log("[saveExtra] result:", res);
      if (res.ok) { setSaveMsg("บันทึกสำเร็จ ✓"); setExtraItems([]); setExtraFilter([]); setShowAdd(false); }
      else setSaveMsg(res.message || "บันทึกไม่สำเร็จ");
                ...
                <div style={{ display: "flex", flexDirection: "column", borderRight: "1px solid #e8ecf0", overflow: "hidden" }}>
                  <div style={{
                    ...
                    <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 6 }}>
                      <span style={{ fontSize: "0.62rem", fontWeight: 700, background: "#0c6075", color: "#fff", padding: "2px 8px", borderRadius: 999 }}>
                        {totalCount} รายการ
                      </span>
                      <button
                        onClick={() => { setShowAdd(v => !v); if (!showAdd) addExtraRow(); }}
                  ...
                  <div style={{ flex: 1, overflowY: "auto", padding: "8px" }}>
                    ...
                    {/* Extra add section */}
                    {showAdd && (
                      <div style={{ borderTop: "1px solid #e2eaf0", padding: "8px", background: "#f9fbfc" }}>
                          ...
                          <div key={idx} style={{ display: "flex", gap: 6, marginBottom: 6, alignItems: "center" }}>
                            <select
                              ...
                              style={{
                                ...
                                border: `1px solid ${val ? "#0c6075" : "#c8d8e4"}`,
                                fontSize: "0.72rem", outline: "none",
                                background: val ? "#f0faf5" : "#fff",
                                color: val ? "#0f7a56" : "#667789",
                                cursor: "pointer",
                              }}
                            >
                              <option value="">— เลือกรายการตรวจ —</option>
                              {testNames.map((n, ni) => (
                                <option key={ni} value={n}>{n}</option>
                              ))}
                            </select>
                            <button
                              onClick={() => removeExtraItem(idx)}
                              style={{ width: 22, height: 22, borderRadius: 4, border: "none", background: "#fee", color: "#c63742", cursor: "pointer", fontSize: 14, fontWeight: 700 }}
                            >×</button>
                          </div>
                        ))}
                        <div style={{ display: "flex", gap: 6, marginTop: 4 }}>
                          <button
                            onClick={addExtraRow}
                            style={{ flex: 1, height: 28, borderRadius: 6, border: "1px dashed #c8d8e4", background: "#fff", color: "#667789", cursor: "pointer", fontSize: "0.7rem", display: "flex", alignItems: "center", justifyContent: "center", gap: 4 }}
                          >
                            <Plus size={12} />เพิ่มอีก
                          </button>
                          <button
                            onClick={saveExtra}
                            disabled={saving || !extraItems.some(v => v.trim())}
                            style={{ flex: 1, height: 28, borderRadius: 6, border: "none", background: saving ? "#ccc" : "#18a779", color: "#fff", cursor: saving ? "not-allowed" : "pointer", fontSize: "0.7rem", fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", gap: 4 }}
                          >
                            <Check size={12} />{saving ? "กำลังบันทึก..." : "ยืนยัน"}
                          </button>
                        </div>
                        {saveMsg && <div style={{ fontSize: "0.65rem", color: "#18a779", marginTop: 4, textAlign: "center" }}>{saveMsg}</div>}
                      </div>
                    )}
                  </div>
                </div>
